...
import { AlphabetCards } from "@/components/summer/AlphabetCards";
...
export const Route = createFileRoute("/_authenticated/student/summer-english/lesson/$order")({
  head: ({ params }) => ({
    meta: [
      { title: `Lesson ${params.order} — Summer English | CRF Online Academy` },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: SummerLessonPage,
});

function SummerLessonPage() {
      ...
      <div className="flex min-h-screen flex-col bg-cream">
        ...
        <div className="grid flex-1 place-items-center px-4 py-16 text-center">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">Enroll in Summer English Lessons for ₦3,000 to unlock it.</p>
            <button
              onClick={() => navigate({ to: "/courses/$slug", params: { slug: "summer-english" } })}
  ...
  return (
    <div className="flex min-h-screen flex-col bg-cream">
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 space-y-6 px-4 py-6 sm:py-8">
          ...
          <section className="rounded-2xl border border-border bg-card p-4 shadow-card sm:p-6">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {lesson.vocabulary.map((v) => (
                <span key={v} className="rounded-full border border-gold/40 bg-gold/10 px-3 py-1 text-sm font-semibold text-navy">
                  {v}
                </span>
              ))}
            </div>
        ...
        {/* Fun activity — alphabet cards (Lesson 1) */}
        {lesson.sort_order === 1 && <AlphabetCards />}
