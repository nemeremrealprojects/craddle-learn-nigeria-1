...
export const Route = createFileRoute("/_authenticated/student/summer-english/")({
  head: () => ({
    meta: [
      { title: "Summer English Dashboard — CRF Online Academy" },
      { name: "description", content: "Your 6-week Summer English Lessons dashboard: progress, lessons and certificate status." },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: SummerDashboard,
});

function SummerDashboard() {
  const { user } = useAuth();
  const { data: course, isLoading: loadingCourse } = useSummerCourse();
  ...
  const stats = summarise(lessons, progress);
    ...
    return (
      <div className="flex min-h-screen flex-col bg-cream">
        ...
        <div className="grid flex-1 place-items-center px-4 py-16 text-center">
          <div>
            <h1 className="font-display text-2xl font-bold text-navy">You're not enrolled in Summer English yet</h1>
            <p className="mt-2 text-muted-foreground">Enroll for ₦3,000 to unlock all six weeks of lessons.</p>
            <Link
              to="/courses/$slug"
              params={{ slug: "summer-english" }}
    ...
    <div className="flex min-h-screen flex-col bg-cream">
      ...
      <section className="bg-hero text-navy-foreground">
        <div className="mx-auto max-w-5xl px-4 py-8 sm:py-10">
          ...
          <h1 className="mt-1 font-display text-3xl font-bold sm:text-4xl">Summer English Lessons</h1>
          ...
          {stats.nextLesson && (
            <Link
              to="/student/summer-english/lesson/$order"
