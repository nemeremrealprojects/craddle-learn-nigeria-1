...
import { P6_READING_SLUG, P6_READING_TOTAL_LESSONS } from "@/lib/p6-reading";
...
export const Route = createFileRoute("/_authenticated/student/p6-reading/")({
  head: () => ({
    meta: [
  ...
  component: P6ReadingDashboard,
});

function P6ReadingDashboard() {
  const { user } = useAuth();
  ...
  const stats = summarise(lessons, progress, P6_READING_TOTAL_LESSONS);
    ...
    return (
      <div className="flex min-h-screen flex-col bg-cream">
        ...
        <div className="grid flex-1 place-items-center px-4 py-16 text-center">
          <div>
            <h1 className="font-display text-2xl font-bold text-navy">You're not enrolled in Primary 6 Reading Skills yet</h1>
            <p className="mt-2 text-muted-foreground">Enroll for ₦3,000 to unlock Week 1 and the rest of the course.</p>
            <Link
              to="/courses/$slug"
              params={{ slug: P6_READING_SLUG }}
    ...
    <div className="flex min-h-screen flex-col bg-cream">
      ...
      <section className="bg-hero text-navy-foreground">
        <div className="mx-auto max-w-5xl px-4 py-8 sm:py-10">
          ...
          <h1 className="mt-1 font-display text-3xl font-bold sm:text-4xl">Primary 6 Reading Skills</h1>
          <p className="mt-2 text-sm text-navy-foreground/80">
            Duration: 12 Weeks · {lessons.length} lesson{lessons.length === 1 ? "" : "s"} available now
          </p>

          <div className="mt-6 flex items-center gap-3">
            <div className="h-3 flex-1 overflow-hidden rounded-full bg-white/15">
              <div className="h-full rounded-full bg-gold-gradient transition-all" style={{ width: `${stats.percent}%` }} />
            </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <InfoCard title="Last lesson viewed">
            {stats.lastViewed ? (
              ...
              <span className="text-muted-foreground">You haven't started a lesson yet — begin with Week 1, Lesson 1.</span>
            )}
          </InfoCard>
          <InfoCard title="Certificate status">
            {stats.certificateReady ? (
              ...
              <span className="text-muted-foreground">
                Locked. Your certificate unlocks when every lesson in the 12-week course is completed, all quizzes passed
                and all assignments submitted.
              </span>
            )}
          </InfoCard>
        </div>
